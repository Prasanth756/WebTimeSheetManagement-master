...
namespace WebTimeSheetManagement.Concrete
{
    public class ProjectConcrete : IProject
    {
        ...
        public ProjectMaster GetProjectDetailsByProjectID(int? ProjectID)
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["TimesheetDBEntities"].ToString()))
            {
                ...
                try
                {
                    ...
                    return con.Query<ProjectMaster>("GetProjectDetails", param, null, true, 0, System.Data.CommandType.StoredProcedure).SingleOrDefault();
                }
                catch (Exception)
                {
                    throw;
        ...
        public List<ProjectMaster> GetListofProjects(int userid)
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["TimesheetDBEntities"].ToString()))
            {
                ...
                {
                    var param = new DynamicParameters();
                    param.Add("@AssignedTOAdmin", userid);
                    var result = con.Query<ProjectMaster>("Usp_getprojectsofAdmin", param, null, true, 0, System.Data.CommandType.StoredProcedure).ToList();
                    if (result.Count > 0)
                    {
                        return result;
                    }
        ...
        public IQueryable<ProjectMaster> ShowProjects(string sortColumn, string sortColumnDir, string Search)
        {
            ...
            {
                IQueryableproject = IQueryableproject.Where(m => m.ProjectName.Contains(Search) || m.ProjectCode.Contains(Search) || m.NatureofIndustry.Contains(Search));
            }
