    ...
    public class UsersConcrete : IUsers
    {
        public IQueryable<RegistrationViewSummaryModel> ShowallUsers(string sortColumn, string sortColumnDir, string Search)
        {
            ...
            var IQueryabletimesheet = (from registration in _context.Registration
                                       join Rolename in _context.Role on registration.RoleID equals Rolename.RoleID
            ...
            {
                IQueryabletimesheet = IQueryabletimesheet.Where(m => m.Name.Contains(Search) || m.AadharNumber.Contains(Search) || m.PanCardNumber.Contains(Search)|| m.EmailID.Contains(Search) || m.Username.Contains(Search) || m.Role.Contains(Search) || m.Mobileno.Contains(Search));
            }
        ...
        public RegistrationViewDetailsModel GetUserDetailsByRegistrationID(int? RegistrationID)
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["TimesheetDBEntities"].ToString()))
            {
                ...
                try
                {
                    ...
                    return con.Query<RegistrationViewDetailsModel>("Usp_GetUserRoleDetailsByRegistrationID", param, null, true, 0, System.Data.CommandType.StoredProcedure).SingleOrDefault();
                }
                catch (Exception)
                {
                    throw;
        ...
        public Registration GetEditableUserDetailsByRegistrationID(int? RegistrationID)
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["TimesheetDBEntities"].ToString()))
            {
                con.Open();
                try
                {
                    var param = new DynamicParameters();
                    param.Add("@RegistrationID", RegistrationID);
                    return con.Query<Registration>("Usp_GetUserRoleDetailsByRegistrationID", param, null, true, 0, System.Data.CommandType.StoredProcedure).SingleOrDefault();
                }
                catch (Exception)
                {
                    throw;
        ...
        public IQueryable<RegistrationViewSummaryModel> ShowallAdmin(string sortColumn, string sortColumnDir, string Search)
        {
            ...
            if (!string.IsNullOrEmpty(Search))
            {
                IQueryabletimesheet = IQueryabletimesheet.Where(m => m.Name.Contains(Search)|| m.Username.Contains(Search) || m.Mobileno.Contains(Search) || m.EmailID.Contains(Search));
            }
        ...
        public IQueryable<RegistrationViewSummaryModel> ShowallUsersUnderAdmin(string sortColumn, string sortColumnDir, string Search, int? RegistrationID)
        {
            ...
            if (!string.IsNullOrEmpty(Search))
            {
                IQueryabletimesheet = IQueryabletimesheet.Where(m => m.Name.Contains(Search) || m.Username.Contains(Search) || m.EmailID.Contains(Search) || m.Mobileno.Contains(Search));
            }
